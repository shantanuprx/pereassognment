package com.assignment.productservice.constants;

/**
 * Constant file to manage string literals related to product service.
 */
public class ProductsConstants {

	public static final String ACTIVE_FLAG = "A";
	public static final String RECORD_CREATION_MESSAGE = "Product created successfully";
	public static final String INVALID_PRODUCT_ID = "Invalid Product Id";
	public static final String YES_FLAG = "Y";
	public static final String DELETED_FLAG = "D";
	public static final String RECORD_UPDATE_MESSAGE = "Product updated Successfully";
	public static final String RECORD_DELETE_MESSAGE = "Product deleted Successfully";
	public static final String ONLY_ADMIN_USER_IS_ALLOWED = "This Operation is allowed only for Admin";
	public static final String OUT_OF_STOCK_STATUS = "O";

	private ProductsConstants() {
		
	}
	
	
}
